...
def test_a_mongo_document_written_before_the_setting_counts_as_on():
    source = open("database.py", encoding="utf-8").read()
    assert 'doc.get("track_inventory", True)' in source
...
def test_turning_it_off_silences_the_unsolicited_alert():
    """The proactive 'you are low' message is the loudest part of the feature."""
    source = open("scheduler.py", encoding="utf-8").read()
    alerts = source[source.index("low_stock_medicines = "):]
    assert "track_inventory" in alerts[:600], "low-stock alerts ignore the setting"


def test_the_reminder_drops_the_stock_line_when_tracking_is_off():
    source = open("scheduler.py", encoding="utf-8").read()
    assert "if show_inventory:" in source
    assert "מלאי נותר" in source
...
def test_pressing_it_while_off_explains_itself():
    source = open("main.py", encoding="utf-8").read()
    assert "מעקב מלאי כבוי" in source
    assert "settings_inventory_toggle" in source
