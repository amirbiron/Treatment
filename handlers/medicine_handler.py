...
(לדוגמה: אקמול, ויטמין D, לבופה וכו')
    ...
    async def get_medicine_name(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            message = f"""
{config.EMOJIS['medicine']} <b>הוספת תרופה: {escape(str(medicine_name))}</b>
...
{config.EMOJIS['medicine']} <b>{escape(str(medicine_data['name']))}</b>
💊 מינון: {escape(str(medicine_data['dosage']))}
...
התזכורות הופעלו אוטומטית!
    ...
    async def view_medicine(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            message = f"""
{config.EMOJIS['medicine']} <b>{escape(str(medicine.name))}</b>

💊 <b>מינון:</b> {escape(str(medicine.dosage))}
⏰ <b>שעות נטילה:</b> {escape(', '.join(schedule_times)) if schedule_times else 'לא מוגדר'}
...
{escape(str(medicine.notes)) if medicine.notes else ''}{inventory_status}
            """
    ...
    async def handle_inventory_update(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if operation == "custom":
                ...
                message = f"""
{config.EMOJIS['inventory']} <b>עדכון מלאי: {escape(str(medicine.name))}</b>
...
{config.EMOJIS['medicine']} {escape(str(medicine.name))}
